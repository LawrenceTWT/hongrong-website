(() => {
  ...
  const zh = {
    ...
    prototypeTdx: "開放資料",
    prototypeData: "剩餘車位",
    prototypeResponse: "語音回覆",
    modelRealityTitle: "比起一個準確率數字，<br><em>真實證據更重要</em>",
    modelSourceCaption: "真實道路攝影機樣本",
    modelLevel1: "非常通暢",
    modelLevel2: "順暢",
    modelLevel3: "一般",
    modelLevel4: "擁擠",
    modelLevel5: "嚴重擁擠",
    modelLevel6: "堵塞",
    modelPredictionCaption: "相鄰車流等級最容易混淆",
    modelCurveCaption: "L4／L5 表現最弱",
    modelSourceNote: "專題原始研究素材 · ResNet-18 · 80% 訓練／20% 驗證",
    modelFindingData: "資料結構已分成六級，但仍需要更多天候、距離與攝影機差異",
    modelFindingClass: "這兩個相鄰交通狀態，是模型最難區分的類別",
    modelFindingDeploy: "Normalization 必須從 PyTorch 到 Android 全程一致",
    modelActionNormalize: "統一 normalization",
    modelActionData: "重新整理資料集",
    modelActionMetrics: "逐類別分析 Precision／Recall",
    ...
    usaPhotoOne: "South Dakota／第一眼",
    usaPhotoTwo: "以前只在照片裡看過的地方",
    usaPhotoThree: "願意耐心聽我說話的人",
    usaPhotoFour: "從不同地方認識的朋友",
    usaPhotoFive: "又去了新的地方",
    usaArrivalTitle: "剛到美國，連吃飯都需要適應",
    usaArrivalBody: "食物又鹹又甜，水有時比汽水還貴；這些小事一直提醒我，我真的離家很遠",
    usaSsnTitle: "申請 SSN 時，我幾乎聽不懂櫃台在說什麼",
    usaSsnBody: "我連問題都不知道該怎麼問，最後靠 employer 協助才完成",
    usaWorkTitle: "在飯店工作，每天都得開口說英文",
    usaWorkBody: "Kaytlin 會耐心理解我想說什麼，也會直接糾正我；這讓我慢慢敢開口",
    courageStatement: "我回到家時，<br><em>帶回了更多勇氣</em>",
    outsideHeading: "那些讓我保持<br><em>好奇與行動的事</em>",
    natureMemory: "一段至今仍留在心裡的記憶",
    natureBody: "退伍後，我和朋友去了紐西蘭，站在 Wānaka 山上時，我只想把眼前的風景記住",
    natureQuote: "看到想去的風景，<br><em>我就會想辦法親自去看看</em>",
    natureImpression: "朋友看到壯觀的景色時，常會說：「這裡洪戎一定會喜歡」",
    skillsStoryHeading: "我用的工具，<br><em>都能在作品裡找到</em>",
    ...
    jinhongSolutionTitle: "技術真正有價值，是讓龐大產品目錄更容易使用",
    jinhongSolutionIntro: "系統把真實產品內容整理成從分類、規格到詢價的清楚路徑",
    jinhongProofCaption: "正式網站產品目錄使用的真實機械照片",
    jinhongTechReactOutcome: "可重用的產品頁面",
    jinhongTechReactNote: "同一份資料結構連接分類、詳細頁、型號與規格",
    jinhongTechRwdOutcome: "每種螢幕都能完成同一件事",
    jinhongTechRwdNote: "手機版仍能清楚導覽、閱讀產品並進入詢價",
    jinhongTechStorageOutcome: "先收藏產品，再進行聯絡",
    jinhongTechStorageNote: "詢價清單不需要帳號或資料庫也能使用",
    jinhongTechDeployOutcome: "建置、部署與持續維護",
    jinhongTechDeployNote: "通過驗證的建置結果部署至 Cloudflare Worker",
    ...
    aboutPageTitle: "因為好奇而開始，<br><em>因為勇氣而前進</em>",
    aboutPageIntro: "我在 AI、Web 與產品之間實作。第一個遺失的網站讓我感受到創造；South Dakota 則讓我學會繼續向前",
    ...
    certTitle: "專業認證",
    nextChapter: "下一個篇章",
    aboutCtaTitle: "一起打造<br><em>真正有用的產品</em>",
    getInTouch: "與我聯絡",
    exploreWork: "瀏覽我的作品",

    workPageEyebrow: "精選作品 — 2026",
    workPageTitle: "從真實需求出發的<br><em>作品</em>",
    workPageIntro: "兩個作品：一個畢業專題、一個真實客戶。點進案例，看見作品背後的決策",
  ...
  const usaCarousel = document.querySelector("[data-usa-carousel]");
  if (usaCarousel) {
    const viewport = usaCarousel.querySelector("[data-usa-carousel-viewport]");
    const slides = [...usaCarousel.querySelectorAll("[data-usa-slide]")];
    const dots = [...usaCarousel.querySelectorAll("[data-usa-carousel-dot]")];
    const previousButton = usaCarousel.querySelector("[data-usa-carousel-prev]");
    const nextButton = usaCarousel.querySelector("[data-usa-carousel-next]");
    const status = usaCarousel.querySelector("[data-usa-carousel-status]");
    let currentSlide = 0;
    let carouselFrame = 0;
    ...
    const moveToSlide = (index, behavior = reducedMotion.matches ? "auto" : "smooth") => {
    ...
    const syncCarouselToScroll = () => {
      ...
      carouselFrame = window.requestAnimationFrame(() => {
        ...
        if (closest.index !== currentSlide) updateCarouselState(closest.index);
      });
    };
    ...
    dots.forEach((dot) => dot.addEventListener("click", () => moveToSlide(Number(dot.dataset.usaCarouselDot))));
    viewport?.addEventListener("scroll", syncCarouselToScroll, { passive: true });
    ...
    window.addEventListener("resize", () => {
      window.cancelAnimationFrame(carouselFrame);
      carouselFrame = window.requestAnimationFrame(() => moveToSlide(currentSlide, "auto"));
    });
    updateCarouselState(0);
    window.requestAnimationFrame(() => moveToSlide(0, "auto"));
  }
